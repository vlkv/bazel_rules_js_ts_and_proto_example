#!/usr/bin/env bash
#
# Taken from https://github.com/aspect-build/rules_lint/blob/v1.7.0/example/lint.sh
#
...
if [ "$#" -eq 0 ]; then
	echo "usage: lint.sh [bazel target pattern...]"
	exit 1
fi
...
# NOTE: These flags are added via .bazelrc
# args=("--aspects=$(echo //tools/lint:linters.bzl%{buf,eslint,flake8,keep_sorted,ruff,shellcheck,clang_tidy} | tr ' ' ',')")
